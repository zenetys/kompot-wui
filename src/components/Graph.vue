<template>
  <div class="ml-5 mr-5 mt-1">
    <v-row>
      <v-col cols="12">
        <v-chip-group mandatory v-model="timeSelected" active-class="primary--text" class="mb-4">
          <v-chip v-for="(item) in timeButtons" :key="item.text" @click="getGraphData(site, item.value)" small outlined label>
            <span>{{ item.text }}</span>
          </v-chip>
        </v-chip-group>
        <v-row>
          <v-col cols="12">
            <v-card class="pa-1" outlined elevation="0">
              <line-chart class="line-chart" :options="dataOptions" :chart-data="fillData()"></line-chart>
            </v-card>
          </v-col>
        </v-row>
      </v-col>
...
export default {
  ...
  data() {
    return {
      ...
      timeButtons: [ {text: 'Année', value: 'year'}, {text: 'Mois', value: 'month'}, {text: 'Semaine', value: 'week'}, {text: 'Jour', value: 'day'},  ] ,
      timeSelected: 2,
      dataCollection: null,
      data: [],
      config: {},
  ...
  methods: {
    ...
    getGraphData(database, start) {
      ...
      axios({
        url: queryurl.setGraphUri(database, timeInterval, datasources.join(",")),
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
        },
